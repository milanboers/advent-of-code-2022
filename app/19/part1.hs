...
qualityLevel bp = Set.findMax . Set.map (\(_, inv) -> has inv Geode) $ endStates
  where
    ...
    endStates = minutes bp startStates 24

findAnswer :: [Blueprint] -> Int
findAnswer = sum . zipWith (\i bp -> i * qualityLevel bp) [1 ..]
