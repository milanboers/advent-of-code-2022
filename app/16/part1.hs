...
data State = State Int Int String (Set String) deriving (Eq, Show, Ord)

nextStates :: Network -> Int -> State -> Set State
nextStates _ 0 _ = Set.empty
nextStates network timeLeft (State endFlow flow valveName closed) =
  Set.unions
    [ stayPutStates,
      tunnelMoveStates,
      openValveStates
  ...
  where
    ...
    stayPutStates = Set.singleton $ State endFlow flow valveName closed
    tunnelMoveStates = Set.map (\tunnel -> State endFlow flow tunnel closed) tunnels
    openValveStates =
      if valveName `Set.notMember` closed
        then Set.empty
        else
          Set.singleton $
            State (endFlow + valveFlow * (timeLeft - 1)) (flow + valveFlow) valveName (Set.delete valveName closed)
...
prune states = values grouped'
  where
    ...
    kvs = Set.map (\s@(State _ f n c) -> ((n, f), s)) states
...
steps network timeLeft states =
  ...
  where
    nextStates' = Set.unions . Set.map (nextStates network timeLeft) $ states
    pruned = prune nextStates'

findAnswer :: Network -> Int
findAnswer network = maximum . Set.map (\(State endFlow _ _ _) -> endFlow) $ finalStates
  where
    finalStates = steps network 30 $ Set.singleton (State 0 0 "AA" (Map.keysSet network))
