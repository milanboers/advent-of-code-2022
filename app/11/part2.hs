...
globalMod :: Monkeys -> Int
globalMod m = product . map ((\(x, _, _) -> x) . test . snd) $ Map.toList m
...
inspect m s mi = case s ! mi of
  ...
  MonkeyState (worryLevel : newMonkeyItems) ins -> newState
    where
      ...
      gm = globalMod m
      newWorryLevel = applyOperation operation' worryLevel `mod` gm
...
findAnswer monkeys = product . take 2 . reverse . sort . map (inspections . snd) . Map.toList $ afterRounds
  where
    ...
    afterRounds = (!! 10000) . iterate (round' monkeys) $ initialState
